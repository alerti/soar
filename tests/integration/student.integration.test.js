...
describe("Student Entity Integration Tests", function () {
  ...
  before(async () => {
    ...
    const {
      body: {
        user: { _id: superadminIdRes },
      },
    } = await request(app).post("/api/users/register").send(superadmin);
    superadminId = superadminIdRes;

    const {
      body: { token: superadminTokenRes },
    } = await request(app)
    ...
    const {
      body: {
        user: { _id: schooladminIdRes },
      },
    } = await request(app).post("/api/users/register").send(schooladmin);
    schooladminId = schooladminIdRes;

    const {
      body: { token: schooladminTokenRes },
    } = await request(app)
      .post("/api/users/login")
      .send({ email: schooladmin.email, password: schooladmin.password });
